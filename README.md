...
A Windows desktop application (VB.NET, .NET 10) that connects to an OPC UA server, monitors live industrial process data in real time, and persists it to a SQL Server database. Built against the OPC Foundation's own reference stack rather than a simplified third-party wrapper library.
...
- **Persistent SQL Server logging** — live data is written to a normalized two-table schema:
...
5. Each change is also written to SQL Server: a new row is appended to `logs`, and the corresponding row in `currentValues` is updated with the latest value and timestamp.
...
5. **Open the project in Visual Studio** and confirm the connection string in `OPCClient.vb` matches your SQL Server instance name and database.
